package com.console.video.view;
...
public class FlingGalleryView extends ViewGroup {
	...
	public FlingGalleryView(Context context, AttributeSet attrs, int defStyle) {
		...
		mCurrentScreen = a
				.getInt(R.styleable.FlingGalleryView_defaultScreen, 1);
		a.recycle();
		init();
	}
	...
	public void computeScroll() {
		...
		if (mScroller.computeScrollOffset()) {
			...
			if (currX < 0) {
				currX = 0;
			} else if (currX > mwholeWidth - getWidth()) {
				currX = mwholeWidth - getWidth();
			}
			scrollTo(currX, 0);
			if (isEveryScreen)
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		for (int i = 0; i < count; i++) {
			...
			if (child.getVisibility() != View.GONE) {
				...
				// final int childWidth =10;
				child.layout(childLeft, 0, childLeft + childWidth,
						child.getMeasuredHeight());
				childLeft += childWidth;
				mwholeWidth = childLeft;
			}
		}
		mwholeWidth = childLeft;
	}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				if (getScrollX() > mwholeWidth - getWidth()) {
					break;
				}
				...
				if ((getScrollX() + deltaX) < 0) {
					deltaX = -getScrollX();
				} else if ((getScrollX() + deltaX) > mwholeWidth - getWidth()) {
					deltaX = mwholeWidth - getWidth() - getScrollX();
				}
				scrollBy(deltaX, 0);
			}
			break;
	...
	private void snapToDestination() {
		...
		final int whichScreen = (getScrollX() + (screenWidth / 2))
				/ screenWidth;
		if (getScrollX() > mwholeWidth - getWidth() - 100) {
			snapToScreen(2, false);
		} else {
	...
	private void snapToScreen(int whichScreen, boolean isJump) {
		...
		if (getScrollX() != (whichScreen * getWidth())) {
			...
			if (whichScreen == 2) {
				delta = mwholeWidth - getWidth() - getScrollX();
			}
			count = -1;
			defaultScreen = -1;
			// 开始滚动动画
	...
	public void setToScreen(int whichScreen, boolean isAnimation) {
		if (isAnimation) {
			...
			mCurrentScreen = whichScreen;
			// 直接滚动到该位置
			int scollDestiny = whichScreen * getWidth();
			if (whichScreen == 2) {
				scollDestiny = mwholeWidth - getWidth();
			}
			scrollTo(scollDestiny, 0);
