package com.console.canreader.fragment.RZCFmilia17;
...
public class AirContorlActivity extends AirConBaseActivity implements OnClickListener,OnTouchListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.ac_controler_rzcfimilia17);
	...
	protected void initPopuptWindow() {
		...
		popupWindowView = getLayoutInflater().inflate(R.layout.ac_more_rzcfimilia17,
				null, false);
		// 点击其他地方消失
	...
	private void initView() {
		...
		REAR_LAMP_INDICATOR.setOnTouchListener(this);
	...
	public void syncView(CanInfo mCaninfo) {
		try {
			...
			if (mCaninfo.CYCLE_INDICATOR == 0) {
				...
				CYCLE_INDICATOR
						.setImageResource(R.drawable.stat_recirculation);
			}
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		try {
			...
			case R.id.CYCLE_INDICATOR:
				if(event.getAction()==MotionEvent.ACTION_DOWN){
					int v6 = mCaninfo.CYCLE_INDICATOR == 0 ? 2 : 1;
					sendMsg("2E8A0205" + BytesUtil.intToHexString(v6));
				}else if(event.getAction()==MotionEvent.ACTION_UP){
					sendMsg("2E8A020500");
				}			
				break;
			case R.id.MAX_FRONT_LAMP_INDICATOR:
				if(event.getAction()==MotionEvent.ACTION_DOWN){
					int v7 = mCaninfo.MAX_FRONT_LAMP_INDICATOR == 0 ? 1 : 2;
					sendMsg("2E8A0207" + BytesUtil.intToHexString(v7));
				}else if(event.getAction()==MotionEvent.ACTION_UP){
					sendMsg("2E8A020700");
				}			
				break;
			case R.id.REAR_LAMP_INDICATOR:
				if(event.getAction()==MotionEvent.ACTION_DOWN){
					int v8 = mCaninfo.REAR_LAMP_INDICATOR == 0 ? 1 : 2;
					sendMsg("2E8A0206" + BytesUtil.intToHexString(v8));
				}else if(event.getAction()==MotionEvent.ACTION_UP){
					sendMsg("2E8A020600");
				}			
				break;
