...
public class RZCBuickGM extends AnalyzeUtils {
	...
	public RZCBuickGM(byte[] msg, int i) {
		super(msg, i);
	}
	...
	public void analyzeEach(byte[] msg, int i) {
		...
		try {
			...
			Trace.i("analyzeEach ==="+(msg[i] & 0xFF));
...
//		mCanInfo.SMALL_LANTERN_INDICATOR = (int) ((msg[3] >> 3) & 0x01);
		...
		if((msg[4]&0x0f)==1||(msg[4]&0x0f)==2||(msg[4]&0x0f)==6||(msg[4]&0x0f)==7||(msg[4]&0x0f)==8||(msg[4]&0x0f)==9){
			mCanInfo.UPWARD_AIR_INDICATOR =1;
		}else{
			mCanInfo.UPWARD_AIR_INDICATOR =0;
		}
		if((msg[4]&0x0f)==1||(msg[4]&0x0f)==4||(msg[4]&0x0f)==5||(msg[4]&0x0f)==6||(msg[4]&0x0f)==9){
			mCanInfo.PARALLEL_AIR_INDICATOR =1;
		}else{
			mCanInfo.PARALLEL_AIR_INDICATOR =0;
		}
		if((msg[4]&0x0f)==1||(msg[4]&0x0f)==3||(msg[4]&0x0f)==4||(msg[4]&0x0f)==8|(msg[4]&0x0f)==9){
	...
	/*
	 * 方向盘按键 STEERING_BUTTON_MODE 0：无按键或释放 1：vol+ 2：vol- 3：menuup 4：menu down 5：SRC
	 * 6SPEECH/answer phone 7:hangup phone
	 */
