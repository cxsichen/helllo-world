...
public class SSChery extends AnalyzeUtils {
 ...
 // 软件版本信息
	...
	private void analyzeControllData(byte[] msg) {
		...
		mCanInfo.FRONT_LAMP_DELAY_STATUS=(int)((msg[6]>>3)&0x01);
		mCanInfo.DAYTIME_LAMP_STATUS=(int)((msg[6]>>2)&0x01);
		mCanInfo.TURN_START_AVM_STATUS=(int)((msg[6]>>1)&0x01);
		mCanInfo.TURN_START_ANIMATION_STATUS=(int)((msg[6]>>0)&0x01);
		
		mCanInfo.SELECTOR_CAR_ASSIST_STATUS=(int)((msg[7]>>6)&0x03);
		mCanInfo.SPEED_OVER_SETTING=(int)(msg[4]&0x1f);
		///////////////仪表背光没做////////////////////
	}
	...
	private void analyzeCarInfoData1(byte[] msg) {
		...
		mCanInfo.DEPUTY_SAFETY_BELT_STATUS=(int)(((msg[6]>>1)&0x01)==0?1:0);
	...
	private void analyzeCarInfoData(byte[] msg) {
		...
		//Log.i("xxx", "mCanInfo.STERRING_WHELL_STATUS=="+mCanInfo.STERRING_WHELL_STATUS);
	...
	void analyzeAirConditionData(byte[] msg) {
		...
		mCanInfo.DEPUTY_DRIVING_POSITON_TEMP = temp == 0xFE ? 0 : temp == 0xFF ? 255
				: (temp * 0.5f);
