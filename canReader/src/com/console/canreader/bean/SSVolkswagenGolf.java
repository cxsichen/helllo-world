...
public class SSVolkswagenGolf extends AnalyzeUtils {
 ...
 // 车身信息
	...
	void analyzeCarInfoData_5(byte[] msg) {
		...
		mCanInfo.CONSUMPTION_SINCE_REFUELING = (int) (msg[4] & 0xff)*256+(int) (msg[5] & 0xff); // 自加油平均油耗
		mCanInfo.RANGE_SINCE_REFUELINGT = (int) (msg[6] & 0xff)*256+(int) (msg[7] & 0xff); // 自加油续航里程	
		mCanInfo.DISTANCE_SINCE_REFUELING = (int) (msg[8] & 0xff)*256+(int) (msg[9] & 0xff); //  自加油行驶里程
		mCanInfo.TRAVELLINGTIME_SINCE_REFUELINGT = (int) (msg[10] & 0xff)*256+(int) (msg[11] & 0xff); //自加油行驶时间
	...
	void analyzeCarInfoData_4(byte[] msg) {
		...
		mCanInfo.CONSUMPTION_LONG_TERM = (int) (msg[4] & 0xff)*256+(int) (msg[5] & 0xff); // 长时间平均油耗
		mCanInfo.RANGE_LONG_TERM = (int) (msg[6] & 0xff)*256+(int) (msg[7] & 0xff); //长时间续航里程	
		mCanInfo.DISTANCE_LONG_TERM = (int) (msg[8] & 0xff)*256+(int) (msg[9] & 0xff); // 长时间行驶里程
		mCanInfo.TRAVELLINGTIME_LONG_TERM = (int) (msg[10] & 0xff)*256+(int) (msg[11] & 0xff); //长时间行驶时间
	...
	void analyzeCarInfoData_1(byte[] msg) {
		...
		mCanInfo.CONSUMPTION_SINCE_START = (int) (msg[4] & 0xff)*256+(int) (msg[5] & 0xff); // 自启动平均油耗
		mCanInfo.RANGE_SINCE_START = (int) (msg[6] & 0xff)*256+(int) (msg[7] & 0xff); // 自启动续航里程	
		mCanInfo.DISTANCE_SINCE_START = (int) (msg[8] & 0xff)*256+(int) (msg[9] & 0xff); // 自启动行驶里程
		mCanInfo.TRAVELLINGTIME_SINCE_START = (int) (msg[10] & 0xff)*256+(int) (msg[11] & 0xff); // 自启动行驶时间
