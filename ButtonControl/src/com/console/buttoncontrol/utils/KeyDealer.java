...
public class KeyDealer {
	...
	public void dealCanKeyEvent(Context context, int i) {
		dealWith(context, i);
	}

	long lastSendTime = 0;

	protected void dealWith(Context context, int i) {
		// TODO Auto-generated method stub
		Log.i("cxs","======dealWith====="+i);
		if (System.currentTimeMillis() - lastSendTime > 500) {
			lastSendTime = System.currentTimeMillis();
			switch (i) {
			case Contacts.K_VOLUP:
				mHandler.removeMessages(Contacts.K_VOLUP);
				mHandler.sendEmptyMessageDelayed(Contacts.K_VOLUP, 200);
				break;
			case Contacts.K_VOLDN:
				mHandler.removeMessages(Contacts.K_VOLDN);
				mHandler.sendEmptyMessageDelayed(Contacts.K_VOLDN, 200);
				break;
			case Contacts.K_AVMUTE:

				mHandler.removeMessages(Contacts.K_AVMUTE);
				mHandler.sendEmptyMessageDelayed(Contacts.K_AVMUTE, 200);
				break;
			case Contacts.K_PREV:
