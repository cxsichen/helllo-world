...
public class SSJeepZNZ17 extends AnalyzeUtils {
	// 数据类型
 ...
 // 车身基本信息
	...
	void analyzeCarInfoData_9(byte[] msg) {
		...
		mCanInfo.REMOTELOCK_BEEP_SIGN = ((int) ((msg[4] >> 5) & 0x01) == 0) ? -1
				: (int) ((msg[5] >> 5) & 0x03);
		// 远程启动提示音
		mCanInfo.REMOTELOCK_SIDELAMP_SIGN = ((int) ((msg[4] >> 6) & 0x01) == 0) ? -1
				: (int) ((msg[5] >> 7) & 0x01);
		// 无钥匙进入
	...
	void analyzeCarInfoData_7(byte[] msg) {
		...
		mCanInfo.CONVENIENCE_SEAT_PARK_MOVE = ((int) ((msg[5] >> 2) & 0x01) == 0) ? -1
				: (int) ((msg[8] >> 3) & 0x01);
		// 影像泊车延时
		mCanInfo.CONVENIENCE_RIPE_PARK_MOVE = ((int) ((msg[5] >> 3) & 0x01) == 0) ? -1
				: (int) ((msg[8] >> 4) & 0x01);
		// 盲点报警
